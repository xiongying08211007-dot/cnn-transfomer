...
#前馈神经网络模块：对输入数据进行非线性变换，以提取不同层次的特征或者进行信息的传递与整合等操作
...
class PatchEmbeddingCNN(nn.Module):
    def __init__(self, f1=16, kernel_size=63, D=2, pooling_size1=8, pooling_size2=8, dropout_rate=0.3, number_channel=52, emb_size=40):
        super().__init__()
        f2 = D * f1
        self.cnn_module = nn.Sequential(
            # temporal conv kernel size 63 (改为奇数)
            nn.Conv2d(2, f1, (1, kernel_size), (1, 1), padding='same', bias=False),  # 将in_channels从1改为2
            nn.BatchNorm2d(f1),
            ...
            # spatial conv
            nn.Conv2d(f2, f2, (1, 15), padding='same', bias=False),  # 将卷积核大小改为奇数
            nn.BatchNorm2d(f2),
            nn.ELU(),
            ...
            nn.Dropout(dropout_rate),
            # 添加一个卷积层将通道数调整为2
            nn.Conv2d(f2, 2, (1, 1), bias=False),
            # 添加上采样层以恢复空间维度
            nn.Upsample(size=(52, 140), mode='bilinear', align_corners=True)
        )

        self.projection = nn.Sequential(
            Rearrange('b e h w -> b e h w'),  # 保持输出形状为 [batch_size, channels, height, width]
        )
...
class fNIRS_T(nn.Module):
    ...
    def __init__(self, n_class, sampling_point, dim, depth, heads, mlp_dim, pool='cls', dim_head=64, dropout=0., emb_dropout=0.):
        ...
        # Add PatchEmbeddingCNN module
        self.patch_embedding_cnn = PatchEmbeddingCNN(f1=16, kernel_size=64, D=2, pooling_size1=8, pooling_size2=8, dropout_rate=0.3, number_channel=52, emb_size=40)
    ...
    def forward(self, img, mask=None):
        print(img.shape)  # 打印输入形状
        img = self.patch_embedding_cnn(img)
        print(img.shape)  # 打印PatchEmbeddingCNN的输出形状
