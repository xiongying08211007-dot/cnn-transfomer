...
writer = SummaryWriter(log_dir='D:/shijie_code/PycharmProjects/CNN+transormer/fNIRS-Transformer-main - A+CNN/fNIRS-Transformer-main/log')
...
# 假设上面的代码部分已经定义了LabelSmoothing、fNIRS_T、fNIRS_PreT等类和函数
...
if __name__ == "__main__":
    ...
    for sub in range(1, Subjects+1):
        # Spilt dataset, one subject's c_preT_data is test set, and the rest is training set.
        if dataset[dataset_id] == 'A':
        ...
        # 创建 SummaryWriter 实例
        writer = SummaryWriter(log_dir=path + '/tb_logs')
        ...
        for epoch in range(EPOCH):
            ...
            writer.add_scalar('Training Accuracy', train_running_acc, epoch)
            ...
            with torch.no_grad():
                ...
                # Write testing loss and accuracy to TensorBoard
                writer.add_scalar('Testing Loss', test_running_loss, epoch)
                writer.add_scalar('Testing Accuracy', test_running_acc, epoch)
        ...
        # Close the SummaryWriter for this subject
        writer.close()