...
def Load_Dataset_A(data_path, model):
    """
    Load Dataset A. Since the dataset does not provide code, we follow the original literature to preprocess the c_preT_data.

    Args:
        data_path (str): dataset path.
        model (str): 'fNIRS-T' or 'fNIRS-PreT'. fNIRS-T uses preprocessed c_preT_data, and fNIRS-PreT uses raw c_preT_data.

    Returns:
        feature : fNIRS signal c_preT_data.
        label : fNIRS labels.
    """
    ...
    for sub in range(1, 9):
        ...
        for i in range(1, trial_num + 1):
            ...
            # fNIRS-T uses preprocessed c_preT_data
            if model == 'fNIRS-T':
...
class Dataset(torch.utils.data.Dataset):
    """
    Load c_preT_data for training

    Args:
        feature: input c_preT_data.
        label: class for input c_preT_data.
